...
func TestCRUD(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("error dialing Mongo %v", err) // Stops the test and errors
	}
	defer sess.Close()
	...
	sess.DB(store.DatabaseName).C(store.CollectionName).RemoveAll(nil) // Delete everything
	//sess.DB(store.DatabaseName).C(store.CollectionName).DropCollection()
}
